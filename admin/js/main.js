...
$(document).ready(function() {
	...
	$('#tipoBuscaTopo').change(function(){
		$('#formBuscaTopo').attr('action', $(this).val());
	});
	$('#formBuscaTopo').attr('action', $('#tipoBuscaTopo').val());
...
*/
	...
	$("#mask-date, .datepicker, .mask-date").mask("99/99/9999");
	$("#mask-cpf").mask("999.999.999-99");
	$("#mask-cnpj").mask("99.999.999/9999-99");
	$("input[name=cepEnderecoPessoa]").mask("99999-999");
	...
	$("#form-validate, #cadastroPessoa").validate({
		rules: {
			required: "required",
			required1: {
				required: true,
		...
		messages: {
			...
			confirmaSenhaPessoa: {
				...
				minlength: "Sua senha deve conter pelo menos 6 caracteres",
				equalTo: "A senha deve ser igual a informada acima"
			},
			emailPessoa: "Informe um e-mail v&aacute;lido",
			confirmaEmailPessoa: {
				required: "Informe seu e-mail novamente",
				equalTo: "O e-mail deve ser igual ao informado acima"
			},
	...
	//coloca conteudo do input para maiusculo
	//inputMaiusculo("input");

	//--formata moeda--//
	$(".mask-4-decimais").maskMoney({decimal:",", thousands:".", precision:3});
	$(".mask-moeda-real").maskMoney({decimal:",", thousands:".", precision:2});
	////

});

//coloca conteudo do input para maiusculo
function inputMaiusculo(input)
{
...
function alertGeral(_type, _title, _text, _icon, _hide){
	new PNotify({
			type: _type,
			title: _title,
			text: _text,
...
function redirectPagina(pagina, chave, valor){
	location.href=pagina+"&"+chave+"="+valor;
}

function geraProdutoSite(idProduto){
	$('body').addClass('loadstate');
	$.post("actions/produto.php", { "acao": "geraProdutoSite", "idProduto": idProduto },
		function(data){
		if(data.cod == 'sucesso'){
...
function populateform(enterlength){
	$('#senhaPessoa, #confirmaSenhaPessoa').val(generatepass(enterlength));
}
/**/
